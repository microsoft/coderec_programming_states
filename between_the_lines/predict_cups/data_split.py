...
def get_features_labels_user_study(path_df, features_to_keep, label_index, REMOVE_S_AND_R, predicting_cups = False):
    ...
    for i in tqdm(range(len(df_observations))):
        ...
        for j in range(len(df_observations[i])):
            ...
            for h in range(len(df_observations[i][j])):
                ...
                if not predicting_cups:
                    label = label_to_idx[np.array(df_observations[i][j][h])[label_index[0]]]
                else:
                    label = get_index(np.array(df_observations[i][j][h])[label_index[0]])
